...
Each script defaults new values to its own private state. The values can target the `host`, 'role', or `run` state by putting a prefix
...
Scripts do not need to use the prefix to access values from the `host`, 'role', or `run` state.
The prefix can be used to only accept values from the target state but this is rarely necessary.
[source,yaml]
----
- sh: echo "hi ${{name}}" #will check default, then host, then role, then run state
