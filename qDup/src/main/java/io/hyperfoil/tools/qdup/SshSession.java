...
public class SshSession {
    ...
    public boolean connect(long timeoutMillis, String setupCommand, boolean trace) {
        ...
        try {
            ...
            sshClient.addSessionListener(new SessionListener() {
                ...
                public void sessionEstablished(Session session) {
                    logger.tracef("%s client established",SshSession.this.getName());
                }

                @Override
                public void sessionCreated(Session session) {
                    logger.tracef("%s client created",SshSession.this.getName());
                }

                @Override
                public void sessionDisconnect(Session session, int reason, String msg, String language, boolean initiator) {
                    logger.tracef("%s client disconnected",SshSession.this.getName());
                }
            ...
            sshClient.setServerKeyVerifier((clientSession1, remoteAddress, serverKey) -> {
                logger.tracef("%s accept server key for {}",SshSession.this.getName(),remoteAddress);
                return true;
            });
            if(host.hasPassword()){
                logger.tracef("%s setting client password provider {}",getName(),identity);
            ...
            if(!RunConfigBuilder.DEFAULT_IDENTITY.equals(identity)){
                logger.tracef("%s setting client identity {}",getName(),identity);
                ...
                try (InputStream inputStream = urlResource.openInputStream()) {
                    ...
                    if(keyPair == null){
                        if(passphrase == RunConfigBuilder.DEFAULT_PASSPHRASE){
                            logger.errorf("%s cannot set client identity %s without a passphrase",getName(),identity);
                        }else{
                            logger.errorf("%s cannot set client identity %s using the provided passphrase",getName(),identity);
                        }
            ...
            if(!future.isConnected()){
                logger.tracef("%s client failed to connect before timeout",SshSession.this.getHost().getHostName());
                return false;
            }
            future = future.verify(this.timeout * 2_000);
            future.await(10,TimeUnit.SECONDS);
            if(!future.isConnected()){
                logger.tracef("%s client failed to verify connection before timeout",SshSession.this.getHost().getHostName());
                return false;
            }
            ...
            clientSession.addSessionListener(new SessionListener() {
                @Override
                public void sessionEstablished(Session session) {
                    logger.tracef("%s session established",SshSession.this.getName());
                }

                @Override
                public void sessionCreated(Session session) {
                    logger.tracef("%s session created",SshSession.this.getName());
                }

                @Override
                public void sessionPeerIdentificationReceived(Session session, String version, List<String> extraLines) {
                    logger.tracef("%s session identification received",SshSession.this.getName());
                }
                ...
                public void sessionNegotiationStart(Session session, Map<KexProposalOption, String> clientProposal, Map<KexProposalOption, String> serverProposal) {
                    logger.tracef("%s session negotiation start",SshSession.this.getName());
                }
                ...
                public void sessionNegotiationEnd(Session session, Map<KexProposalOption, String> clientProposal, Map<KexProposalOption, String> serverProposal, Map<KexProposalOption, String> negotiatedOptions, Throwable reason) {
                    logger.tracef("%s session negotiation end",SshSession.this.getName());
                }
                ...
                public void sessionException(Session session, Throwable t) {
                    logger.tracef("%s session exception: %s",SshSession.this.getName(),t.getMessage());
                }

                @Override
                public void sessionDisconnect(Session session, int reason, String msg, String language, boolean initiator) {
                    logger.tracef("%s session disconnect",SshSession.this.getName());
                }

                @Override
                public void sessionClosed(Session session) {
                    logger.tracef("%s session closed",SshSession.this.getName());
                }
            });

            if(RunConfigBuilder.DEFAULT_PASSPHRASE==passphrase){
                logger.tracef("%s using %s identity without passphrase",getName(),identity);
            }else{
                logger.tracef("%s using %s identity with a passphrase",getName(),identity);
            }
            URLResource urlResource = new URLResource(Paths.get(identity).toUri().toURL());
            try (InputStream inputStream = urlResource.openInputStream()) {
                Iterable<KeyPair> keyPairs = SecurityUtils.loadKeyPairIdentities(
                ...
                if(keyPair == null){
                    if(passphrase == RunConfigBuilder.DEFAULT_PASSPHRASE){
                        logger.errorf("cannot connect %s using %s without a passphrase",getName(),identity);
                    }else{
                        logger.errorf("cannot connect %s using %s using the provided passphrase",getName(),identity);
                    }
                    return false; // we failed to connect
                }
            ...
            if (host.hasPassword()) {
                logger.tracef("%s adding password-identity",SshSession.this.getName());
            ...
            logger.tracef("%s creating channel shell",getName());
            ...
            if (timeoutMillis > 0) {
                logger.tracef("%s opening and verifying channel shell with {} timeout",getName(),timeoutMillis);
            ...
            try {
                ...
                logger.warnf("%s@%s interrupted while waiting for initial PROMPT", host.getUserName(), host.getHostName());
                Thread.interrupted();
            ...
            logger.error("Security exception while connecting to "+host.getSafeString()+" using "+identity+"\n"+e.getMessage(), e);
        } catch (IOException e) {
            //e.printStackTrace();
            logger.error("Exception while connecting to "+host.getSafeString()+" using using "+identity+"\n"+e.getMessage(), e);
        } finally {
            logger.tracef("%s session.isOpen=%s shell.isOpen=%s",
                    this.getName(),
        ...
        logger.tracef("%s connect returning %s",
                this.getName(),
                rtrn
        );
        return rtrn;
...
//            }
    ...
    private void sh(String command, boolean acquireLock, BiConsumer<String,String> callback, Map<String, String> prompt) {
        ...
        logger.tracef("%s sh: %s, lock: %s", host, command, acquireLock);
                ...
                if (wait) {
                    try {
                        if (shellLock.availablePermits() <= 0) {
                            logger.infof("%s closing but shell still locked %s", this.getHost(), lastCommand);
                        }
                        shellLock.acquire();
                        if (permits() != 0) {
