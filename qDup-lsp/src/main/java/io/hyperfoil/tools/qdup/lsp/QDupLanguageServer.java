...
public class QDupLanguageServer implements LanguageServer, LanguageClientAware {
    ...
    public QDupLanguageServer() {
        ...
        this.workspaceService.setFileChangeHandler(textDocumentService::handleFileChange);
    }
    ...
    public CompletableFuture<InitializeResult> initialize(InitializeParams params) {
        ...
        ServerInfo serverInfo = new ServerInfo("qDup Language Server", getVersion());
    ...
    private String getVersion() {
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("lsp-version.properties")) {
            if (is != null) {
                Properties versionProps = new Properties();
                versionProps.load(is);
                return versionProps.getProperty("lsp.version", "unknown");
            }
        } catch (IOException e) {
            LOG.log(Level.FINE, "Failed to read lsp-version.properties", e);
        }
        return "unknown";
    }
