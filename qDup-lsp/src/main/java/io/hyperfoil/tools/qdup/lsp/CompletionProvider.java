...
public class CompletionProvider {
    ...
    public List<CompletionItem> complete(QDupDocument document, int line, int character) {
        ...
        return completeForContext(context, document, line, null);
    }
    ...
    public List<CompletionItem> complete(QDupDocument document, int line, int character, Collection<QDupDocument> allDocs) {
        ...
        YamlContext context = contextResolver.resolve(document, line, character);
        return completeForContext(context, document, line, allDocs);
    }
     ...
     */
    public List<CompletionItem> completeForContext(YamlContext context, QDupDocument document, int line, Collection<QDupDocument> allDocs) {
        List<CompletionItem> items = new ArrayList<>();
        ...
        switch (context) {
            ...
            case ROLE_SCRIPT_REF:
                Set<String> scriptNamesSeen = new LinkedHashSet<>();
                collectNames(document.getScriptNames(), scriptNamesSeen, items, "Script", CompletionItemKind.Reference);
                if (allDocs != null) {
                    for (QDupDocument other : allDocs) {
                        if (!other.getUri().equals(document.getUri()) && other.isParseSuccessful()) {
                            collectNames(other.getScriptNames(), scriptNamesSeen, items, "Script", CompletionItemKind.Reference);
                        }
                    }
                }
                break;
            ...
            case ROLE_HOST_REF:
                Set<String> hostNamesSeen = new LinkedHashSet<>();
                collectNames(document.getHostNames(), hostNamesSeen, items, "Host", CompletionItemKind.Reference);
                if (allDocs != null) {
                    for (QDupDocument other : allDocs) {
                        if (!other.getUri().equals(document.getUri()) && other.isParseSuccessful()) {
                            collectNames(other.getHostNames(), hostNamesSeen, items, "Host", CompletionItemKind.Reference);
                        }
                    }
                }
                break;
    ...
    private void collectNames(Set<String> names, Set<String> seen, List<CompletionItem> items, String label, CompletionItemKind kind) {
        for (String name : names) {
            if (seen.add(name)) {
                CompletionItem item = new CompletionItem(name);
                item.setKind(kind);
                item.setDetail(label + ": " + name);
                items.add(item);
            }
        }
    }
